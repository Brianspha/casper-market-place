...
export default {
  ...
  methods: {
    ...
    init2: async function() {
      ...
      this.board = grid;
      this.$store.state.boardOwned = this.board;
      this.$store.state.gridOwned = this.grid;
      ...
      for (var cell in this.$store.state.gridOwned) {
        //  console.log("celllll: ", this.$store.state.grid[cell]);
        var innerCells = [];
        for (var innerGridCell in this.$store.state.gridOwned[cell]) {
          var gridCell = this.$store.state.gridOwned[cell][innerGridCell];
          if (colorPixels.includes(gridCell.id.toString())) {
            ...
            if (gridCell.owner === _this.$store.state.userAddress) {
